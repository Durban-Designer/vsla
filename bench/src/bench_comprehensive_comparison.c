...
static void benchmark_sparsity_scenarios(vsla_context_t* ctx) {
    ...
    for (int s = 0; s < num_levels; s++) {
        ...
        cblas_dcopy(base_size, a_dense, 1, out_dense, 1);
        cblas_daxpy(base_size, 1.0, b_dense, 1, out_dense, 1);
        ...
        start = get_time();
        for (int r = 0; r < runs; r++) {
            cblas_dcopy(base_size, a_full, 1, out_full, 1);
            cblas_daxpy(base_size, 1.0, b_full, 1, out_full, 1);
        }
...
static void benchmark_matrix_operations(vsla_context_t* ctx) {
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        cblas_dgemv(CblasRowMajor, CblasNoTrans, m, n, 1.0, matrix_blas, n, vector_blas, 1, 0.0, result_blas, 1);
...
static void benchmark_performance_breakdown(vsla_context_t* ctx) {
    ...
    start = get_time();
    for (int r = 0; r < runs; r++) {
        cblas_dcopy(size, a_data, 1, out_data, 1);
        cblas_daxpy(size, 1.0, b_data, 1, out_data, 1);
    }
    ...
    printf("%20s %12.2f %12.1f%% %12s\n", "Pure_computation", compute_time, 
           (compute_time / total_time) * 100, "BLAS_optimal");
