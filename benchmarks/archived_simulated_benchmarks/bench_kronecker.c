...
// Simple Kronecker (using outer product approach)
static void simple_kronecker(const double* a, size_t m, const double* b, size_t n, double* out) {
    // For each element of a, multiply it by the entire vector b
    for (size_t i = 0; i < m; i++) {
        simple_dcopy(n, b, &out[i * n]);
        simple_dscal(n, a[i], &out[i * n]);
    }
}
...
static void benchmark_kronecker_sizes(vsla_context_t* ctx) {
    ...
    printf("%8s %8s %12s %12s %12s %10s %12s\n", 
           "Size_A", "Size_B", "VSLA(ms)", "Manual(ms)", "Simple(ms)", "Ratio_M", "Ratio_S");
    printf("---------------------------------------------------------------------------------\n");
    ...
    for (int tc = 0; tc < num_cases; tc++) {
        ...
        for (int w = 0; w < warmup_runs; w++) {
            ...
            simple_kronecker(a_data, a_size, b_data, b_size, out_blas);
        }
...
static void benchmark_scalar_cases(vsla_context_t* ctx) {
    ...
    printf("%12s %12s %12s %12s %15s\n", 
           "Vector_Size", "VSLA(μs)", "Simple(μs)", "Ratio", "Advantage");
    printf("----------------------------------------------------------\n");
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        simple_dcopy(vec_size, vec_data, out_blas);
        simple_dscal(vec_size, scalar_val, out_blas);
        ...
        const char* advantage = (ratio < 1.1) ? "VSLA" : (ratio < 2.0) ? "Comparable" : "Simple";
